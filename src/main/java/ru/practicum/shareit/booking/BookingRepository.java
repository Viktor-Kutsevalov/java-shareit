...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findCompletedBookingsForComment(@Param("userId") Long userId,
                                                  @Param("itemId") Long itemId,
                                                  @Param("now") LocalDateTime now);

    @Query("select b from Booking b join fetch b.item i join fetch b.booker u " +
            "where b.item.id = :itemId and b.status = 'APPROVED' and b.start < :now order by b.start desc")
    List<Booking> findLastApprovedByItemId(@Param("itemId") Long itemId, @Param("now") LocalDateTime now);

    @Query("select b from Booking b join fetch b.item i join fetch b.booker u " +
            "where b.item.id = :itemId and b.status = 'APPROVED' and b.start > :now order by b.start asc")
    List<Booking> findNextApprovedByItemId(@Param("itemId") Long itemId, @Param("now") LocalDateTime now);

    @Query("select b from Booking b join fetch b.item i join fetch b.booker u " +
            "where b.item.id in :itemIds and b.status = 'APPROVED' and b.start < :now order by b.start desc")
    List<Booking> findLastApprovedByItemIdIn(@Param("itemIds") List<Long> itemIds,
                                             @Param("now") LocalDateTime now,
                                             Sort sort);

    @Query("select b from Booking b join fetch b.item i join fetch b.booker u " +
            "where b.item.id in :itemIds and b.status = 'APPROVED' and b.start > :now order by b.start asc")
    List<Booking> findNextApprovedByItemIdIn(@Param("itemIds") List<Long> itemIds,
                                             @Param("now") LocalDateTime now,
                                             Sort sort);
}
